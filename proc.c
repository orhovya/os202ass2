...
exit(void)
{
  ...
  while (!cas(&curproc->state, -1, -ZOMBIE));
  sched();
  panic("zombie exit");
}
...
sched(void)
{
  ...
  while (!cas(&intena, -1,  mycpu()->intena));

  swtch(&p->context, mycpu()->scheduler);

  while (!cas(&(mycpu()->intena), -1, intena));
...
yield(void)
{
  ...
  sched();
  while (!cas(& (myproc()->state), -RUNNABLE, RUNNABLE));
  popcli();
}
